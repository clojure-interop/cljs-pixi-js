(ns pixi.SimplePlane
  ...
  The SimplePlane allows you to draw a texture across several points
  then manipulate these points
  ```js
  for (let i = 0; i < 20; i++) {
  points.push(new PIXI.Point(i * 50, 0));
  };
  let SimplePlane = new PIXI.SimplePlane(PIXI.Texture.from(\"snake.png\"),
  ```"
  (:refer-clojure :exclude []))

(defn new
  "Constructor."
  ([texture vertices-x vertices-y]
    (new js/SimplePlane texture vertices-x vertices-y)))

(defn calculate-uvs
  "Method.

  Member of SimplePlane.
